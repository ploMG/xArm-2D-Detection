...
import cv2
import numpy as np
from main import x_real, y_real, z_real, xmin, ymin, ymax, zmin, zmax, cz, cam_x,\
...
# Run second loop for the height of the detected object
while tr != object_name:
    ...
    if ret:
        ...
        # YOLOv5 detection on exported image
        os.system(f'cmd /c "{yolo_main_path[:2]} & cd {yolo_main_path.replace("/", chr(92))} & python detect.py --source {export_path.replace("/", chr(92))}"')
...
print(arm.set_position(x=250, y=0, z=rZ, roll=180, pitch=0, yaw=0, is_radian=False, wait=True))
arm.clean_error()
time.sleep(0.5)
arm.set_state(0)